...
1. 用户管理
    ...
    1. 角色的位置放到启用前                         -- 已修复
    1. 用户角色没有保存成功（新增）                  --已修复
    1. 增加角色时，同步到工作流用户组                --已修复
    
1. 通讯录中公司和部门加上必须校验                      --已修复（未复现）
    1. 通讯录列表选择部门后，点添加，没有带部门到新增页面 --已修复
    1. 从新增页面取消后，列表页部门不可用   ----非必要bug
    
1. 新增部门，检查所属区域的错误消息，所属公司-》所属部门  --非必要bug
    1. 部门列表的公司树状列表，点+会关闭树              --已修复
    
1. 区域管理列表的清空，没有清空树的选中状态 --非bug

1. 需求单里项目补充编码不应该是必须填 ----已修复
    1. 项目编号隐藏 ----已修复
    1. 销售员初次填的时候，投放地区等没有控制住----没有复现
    1. 销售单更新时，person没有判断存在，会出空指针异常 --已修复
    1. 是否加数据权限控制 ---待定
